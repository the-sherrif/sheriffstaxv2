'use client';
/* Adapted from Bencho Checklist, Copyright (c) 2026 Lorenzo Cabra.
   MIT licence: see licences/bencho-MIT.txt. Original explanatory comments retained. */
...
export function Checklist({
  ...
  const [demoItems, setItems] = useState(() =>
  ...
  const items = tasks ?? demoItems;
  const [adding, setAdding] = useState(false);
  const [draft, setDraft] = useState("");
  const panelId = useId();
  const [openOrder, setOpenOrder] = useState<string | null>(() => tasks?.find(t => !t.done)?.id ?? tasks?.[0]?.id ?? null);
  ...
  const fell = !tasks && items.length > 0 && items.every((t) => t.done);
  ...
  const spare = !tasks && items.length < MAX;
  ...
  /* Field Orders are controlled application data, not the demo on a wall.
     Keep the spring per row, but never add/reset tasks or collapse the form:
     the evidence fields must remain reachable after completion and on reload. */
  if (tasks) return (
    <div className="chk chk-orders" role="group" aria-label="Field Orders" style={{ borderRadius: r }}>
      {/* Keep evidence mounted while folded: changing orders must not discard a draft.
          The disclosure is separate from the checkbox, so reading is not completion. */}
      {tasks.map((task, i) => <div className="chk-order" key={task.id}>
        <div className="chk-order-heading">
          <Row label={task.text} on={task.done} side={side} bounce={bounce}
            still={still} fell={false} drop={0} secs={0} inset={0} drift={0}
            tilt={0} index={i} onRun={measure} onToggle={() => {
              if (task.readOnly) {
                setOpenOrder(task.id);
                requestAnimationFrame(() => task.onToggle());
              } else task.onToggle();
            }} disabled={task.disabled} readOnly={task.readOnly}/>
          <button type="button" className="chk-disclosure"
            aria-expanded={openOrder === task.id} aria-controls={`${panelId}-${task.id}`}
            aria-label={`${openOrder === task.id ? "Hide" : "Show"} instructions: ${task.text}`}
            onClick={() => setOpenOrder(v => v === task.id ? null : task.id)}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true"><path d={openOrder === task.id ? "m6 15 6-6 6 6" : "m6 9 6 6 6-6"}/></svg>
          </button>
        </div>
        <div className="chk-details" id={`${panelId}-${task.id}`} hidden={openOrder !== task.id}>{task.details}</div>
      </div>)}
    </div>
  );
...
function Row({
  ...
  /* Drive the spring in percent units as its snap-threshold comment requires,
     then normalise once for all four visual readings. */
  const t = useSpring(on ? 100 : 0, clamp(bounce, 0, 100), still) / 100;
