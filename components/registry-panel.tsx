...
export function RegistryPanel() {
  ...
  let linkCount=0;
  if(app){for(const [url,repost] of [[orders.spreadUrl,orders.spreadMode==='repost'],[orders.replyUrl,false]] as const){try{const p=parsePostUrl(url);if(repost?app.warrantUrl&&p.id===parsePostUrl(app.warrantUrl).id:p.handle===app.handle&&(!app.warrantUrl||p.id!==parsePostUrl(app.warrantUrl).id))linkCount++;}catch{}}}
  const completed=Number(orders.follow)+Number(orders.like)+linkCount;
  const ready=(()=>{if(!app)return false;try{validateOrders(orders,app.handle,app.warrantUrl);return true;}catch{return false;}})();
  return <div className={`application-card ${app?'has-application':''}`}>
    {!loaded&&<p role="status" className="mono">OPENING THE REGISTRY…</p>}
    {loaded&&!app&&!deputy&&<><p className="eyebrow">YOUR FIRST ASSIGNMENT</p><h3>Apply for a badge.</h3><p>Enter your X username. The Sheriff has a few orders before making it official.</p><form onSubmit={e=>{e.preventDefault();void run(async()=>{await apply(handle);});}}><label className="field-label" htmlFor="applicant-handle">YOUR X USERNAME</label><div className="handle-field"><span>@</span><input id="applicant-handle" autoComplete="username" autoCapitalize="none" spellCheck={false} maxLength={16} placeholder="yourusername" value={handle} onChange={e=>setHandle(e.target.value)} required/></div><button className="button primary full" disabled={busy||!handle.trim()} type="submit">{busy?'SAVING APPLICATION…':'RECEIVE YOUR FIELD ORDERS'}<ArrowRight size={18}/></button></form><p className="form-footnote">Free to apply. No wallet required.</p><button className="text-link" type="button" onClick={()=>setRecoverOpen(true)}>ALREADY HAVE A BADGE? RECOVER IT <RotateCcw size={13}/></button></>}
    {app&&!deputy&&<><div className="orders-top"><p className="eyebrow">WARRANT #{app.warrantNumber} / DEPUTY RECRUITMENT</p><span className="applicant-label">APPLICANT</span></div><h3 className="orders-title">Sheriff’s<br/>Field Orders.</h3><p className="applicant-name">@{app.handle}</p><p>A badge has to be earned. Complete your orders before the Sheriff deputizes you.</p>
      ...
      <div className="field-order"><span className="order-number">01</span><div><h4>FOLLOW THE SHERIFF</h4><a className="order-action" href={`https://x.com/${campaign.sheriffHandle}`} target="_blank" rel="noreferrer">Follow @{campaign.sheriffHandle}<ArrowUpRight size={15}/></a><label className="completion-label"><Checkbox checked={orders.follow} onCheckedChange={v=>setOrders({...orders,follow:v})} aria-label="I have followed the Sheriff"/>I’ve completed this</label></div></div>
      <div className="field-order"><span className="order-number">02</span><div><h4>LIKE THE WARRANT</h4>{app.warrantUrl?<a className="order-action" href={app.warrantUrl} target="_blank" rel="noreferrer">Open the official Warrant<ArrowUpRight size={15}/></a>:<span className="order-unavailable">Official post coming soon</span>}<label className="completion-label"><Checkbox checked={orders.like} disabled={!app.warrantUrl} onCheckedChange={v=>setOrders({...orders,like:v})} aria-label="I have liked the Warrant"/>I’ve completed this</label></div></div>
      <div className="field-order"><span className="order-number">03</span><div><h4>SPREAD THE WARRANT</h4><p className="order-help">Repost it, or add your own words in a quote.</p><RadioGroup value={orders.spreadMode} onValueChange={v=>setOrders({...orders,spreadMode:v as 'quote'|'repost',spreadUrl:''})} className="spread-options" aria-label="Choose how to spread the Warrant"><label><RadioGroupItem value="quote"/>QUOTE POST</label><label><RadioGroupItem value="repost"/>REPOST</label></RadioGroup>{app.warrantUrl&&<a className="order-action" href={app.warrantUrl} target="_blank" rel="noreferrer">Open Warrant on X<ArrowUpRight size={15}/></a>}<label className="field-label" htmlFor="spread-url">{orders.spreadMode==='quote'?'YOUR QUOTE POST LINK':'REPOSTED WARRANT LINK'}</label><input id="spread-url" type="url" inputMode="url" placeholder={orders.spreadMode==='quote'?`https://x.com/${app.handle}/status/…`:'Paste the original Warrant link'} value={orders.spreadUrl} onChange={e=>setOrders({...orders,spreadUrl:e.target.value})}/>{orders.spreadMode==='repost'&&<p className="order-help">A plain repost keeps the Sheriff’s original link. Paste that link after reposting.</p>}<LinkStatus value={orders.spreadUrl} handle={app.handle} repost={orders.spreadMode==='repost'} warrantUrl={app.warrantUrl}/></div></div>
      <div className="field-order"><span className="order-number">04</span><div><h4>REPORT FOR DUTY</h4><p className="reply-prompt">“Tell the Sheriff why you deserve a badge.”</p>{app.warrantUrl&&<a className="order-action" href={app.warrantUrl} target="_blank" rel="noreferrer">Reply underneath the Warrant<ArrowUpRight size={15}/></a>}<label className="field-label" htmlFor="reply-url">YOUR COMMENT LINK</label><input id="reply-url" type="url" inputMode="url" placeholder={`https://x.com/${app.handle}/status/…`} value={orders.replyUrl} onChange={e=>setOrders({...orders,replyUrl:e.target.value})}/><LinkStatus value={orders.replyUrl} handle={app.handle} warrantUrl={app.warrantUrl}/></div></div>
